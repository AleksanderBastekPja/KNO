...
if __name__ == "__main__":
    ...
    df = pd.read_csv("lab3/wine/wine.data", header=None, names=cols)
    ...
    history_m2 = model_2.fit(X_train, y_train, epochs=int(epochs_m2), batch_size=batch_size_m2)
    model_2.evaluate(X_test, y_test)

    plt.plot(history_m1.history['accuracy'], label='Training Accuracy m1')
    plt.plot(history_m2.history['accuracy'], label='Training Accuracy m2')

    if 'val_accuracy' in history_m1.history:
        plt.plot(history_m1.history['val_accuracy'], label='Validation Accuracy for m1')
    if 'val_accuracy' in history_m2.history:
        plt.plot(history_m2.history['val_accuracy'], label='Validation Accuracy for m2')

    plt.title('Model Accuracy over Epochs for model 2')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.legend()
    plt.grid(True)
